...
void WaitState_Cyclic(WaitState* const pThis)
{
    ...
    {
        uint32_t currentTimeUs = DrvTimer_GetCurrentValue(
                                     pThis->pCfg->pDrvTimer);

        uint32_t reloadValueUs = DrvTimer_GetReloadValue(
                                     pThis->pCfg->pDrvTimer);

        if (reloadValueUs != 0UL)
        {
            pThis->data.cpuLoadPercent = (((float) currentTimeUs) * 100.0f)
                                         / ((float) reloadValueUs);
        }
